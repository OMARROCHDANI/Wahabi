...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-orange-500 selection:text-black">
      ...
      {/* Mobile Menu */}
      <div className={`fixed inset-0 z-40 bg-orange-600 transition-transform duration-700 ease-in-out ${isMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className="flex flex-col items-center justify-center h-full space-y-8 text-4xl font-black italic">
          {navLinks.map((link) => (
      ...
      <section id="accueil" className="relative min-h-screen flex items-center pt-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-8">
            <h2 className="text-orange-500 font-bold tracking-[0.3em] uppercase mb-4 animate-fade-in">Logistique & Manutention</h2>
            <h1 className="text-6xl md:text-8xl lg:text-9xl font-black leading-tight tracking-tighter mb-8 italic">
            ...
            <div className="flex flex-wrap gap-4">
              <button onClick={() => scrollTo('contact')} className="px-8 py-4 bg-orange-500 text-black font-bold uppercase tracking-widest flex items-center gap-3 hover:bg-white transition-all transform hover:-translate-y-1">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="p-8 border-l-4 border-orange-500 bg-white/5">
              <p className="text-3xl font-black text-white italic">"Rigueur, Sécurité, Réactivité."</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            <div>
              ...
              <h2 className="text-5xl md:text-6xl font-black mt-4 mb-8 uppercase italic leading-tight">
              ...
              <div className="grid grid-cols-2 gap-8 mt-12">
                {[
                  ...
                  <div key={i} className="border-t border-black/10 pt-4">
                    <h4 className="font-black text-xl uppercase italic">{item.title}</h4>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            ...
            <div className="md:col-span-2 group relative p-10 bg-white/5 rounded-3xl border border-white/10 hover:border-orange-500 transition-all duration-500 overflow-hidden">
              ...
              <div className="flex items-start justify-between relative z-10">
                <div>
                  ...
                  <h3 className="text-4xl font-black uppercase mb-4 italic">Conduite CACES</h3>
                  <p className="text-gray-400 mb-8 max-w-md">Maîtrise totale des chariots élévateurs avec respect strict des normes de sécurité.</p>
                </div>
                <div className="text-right">
                  <span className="text-5xl font-black text-orange-500 italic">R485/489</span>
                </div>
              </div>
            ...
            <div className="p-10 bg-white/5 rounded-3xl border border-white/10 hover:border-orange-500 transition-all group">
              ...
              <h3 className="text-2xl font-black uppercase mb-4 italic">Gestion Magasin</h3>
              <ul className="space-y-4 text-gray-400 font-medium">
                <li className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-orange-500" /> Réception de marchandises</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-orange-500" /> Déchargement sécurisé</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-orange-500" /> Mise en stock optimisée</li>
              </ul>
            </div>
          </div>
      ...
      <section id="certifications" className="py-32 bg-orange-600 text-black">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-4 mb-16">
            ...
            <h2 className="text-2xl font-black uppercase tracking-[0.4em] shrink-0">Formation & Diplômes</h2>
            <div className="h-px flex-grow bg-black/20"></div>
          </div>
          ...
          <div className="space-y-6">
            {certifications.map((cert, index) => (
              <div key={index} className="group bg-black text-white p-8 md:p-12 rounded-[2rem] flex flex-col md:flex-row gap-8 items-center transition-all hover:scale-[1.02] cursor-default">
                <div className="w-20 h-20 bg-orange-500 rounded-2xl flex items-center justify-center text-black shrink-0">
                  {cert.icon}
                </div>
                <div className="flex-grow text-center md:text-left">
                  <div className="flex flex-col md:flex-row md:items-center gap-4 mb-2">
                    <h3 className="text-3xl font-black uppercase italic leading-none">{cert.title}</h3>
                    <span className="px-4 py-1 bg-white/10 rounded-full text-xs font-bold self-center md:self-start">{cert.date}</span>
                  </div>
                  <p className="text-orange-500 font-bold mb-4 uppercase text-sm tracking-widest">{cert.org}</p>
                  <p className="text-gray-400 text-lg">{cert.desc}</p>
                </div>
      ...
      <section id="contact" className="py-32 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h2 className="text-7xl md:text-[10rem] font-black italic tracking-tighter mb-20 leading-none opacity-10 uppercase select-none">Contact</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
            <div className="text-left">
              <h3 className="text-4xl font-black uppercase italic mb-8">Commençons à <span className="text-orange-500 underline underline-offset-8">travailler ensemble</span></h3>
              <p className="text-xl text-gray-400 mb-12 leading-relaxed">
                Disponible immédiatement pour des opportunités en logistique et gestion d'entrepôt sur toute la région.
              </p>
              
              <div className="space-y-6">
                <a href="tel:+212700354347" className="flex items-center gap-6 group w-fit">
                  <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-orange-500 group-hover:text-black transition-all duration-300">
                    <Phone size={20} />
                  </div>
                  <span className="text-xl md:text-2xl font-bold group-hover:text-orange-500 transition-colors">+212 700 354 347</span>
                </a>
                <a href="mailto:mouradwahabi008@gmail.com" className="flex items-center gap-6 group w-fit">
                  <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-orange-500 group-hover:text-black transition-all duration-300">
                    <Mail size={20} />
                  </div>
                  <span className="text-xl md:text-2xl font-bold truncate group-hover:text-orange-500 transition-colors">mouradwahabi008@gmail.com</span>
                </a>
                <div className="flex items-center gap-6 group w-fit">
                  <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-orange-500">
                    <MapPin size={20} />
                  </div>
                  <span className="text-xl md:text-2xl font-bold">Oued Zem, Maroc</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-orange-500 p-8 md:p-12 rounded-[2.5rem] text-black text-left shadow-2xl shadow-orange-500/20 transform hover:scale-[1.01] transition-transform">
              ...
              <div className="space-y-4">
                <a href="mailto:mouradwahabi008@gmail.com" className="w-full py-5 bg-black text-white font-black uppercase text-sm tracking-[0.15em] flex items-center justify-center gap-3 hover:bg-zinc-900 transition-colors rounded-2xl group whitespace-nowrap overflow-hidden">
                   Envoyer un Email <Send className="w-4 h-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                </a>
                <a href="tel:+212700354347" className="w-full py-5 border-2 border-black text-black font-black uppercase text-sm tracking-[0.15em] flex items-center justify-center gap-3 hover:bg-black hover:text-white transition-all rounded-2xl whitespace-nowrap overflow-hidden">
                  Appeler Maintenant
                </a>
              </div>
            </div>
      ...
      <footer className="py-12 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
          <p className="text-gray-500 font-medium tracking-widest text-xs uppercase">© 2024 MOURAD WAHABI — CARISTE CERTIFIÉ</p>
          <div className="flex gap-8 md:gap-12 text-xs font-black italic uppercase tracking-tighter">
            <span className="hover:text-orange-500 cursor-pointer transition-colors">LinkedIn</span>
            <span onClick={() => scrollTo('contact')} className="hover:text-orange-500 cursor-pointer transition-colors">Contact</span>
            <span className="hover:text-orange-500 cursor-pointer transition-colors">CV PDF</span>
          </div>
        </div>
      </footer>
    </div>
